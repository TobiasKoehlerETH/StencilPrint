...
1. `index.html` provides the static interface and `src/main.tsx` wires its controls.
...
3. Tauri dispatches `preview_stencil`, `save_stencil_step`, or `export_stencil_stl` in `src-tauri/src/lib.rs`.
...
6. Preview returns the shared polygon model; STEP export passes the same geometry to `step.rs`, which builds watertight B-Reps and delegates AP203 serialization to `brepkit`; STL export triangulates the same profiles in `stl.rs`.
